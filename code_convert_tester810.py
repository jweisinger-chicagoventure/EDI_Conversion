...
for i, row in enumerate(pd_order.iterrows()):
    ...
    curr_segment = ["BIG", str(curr_date.strftime('%Y%m%d')), orderid_var] + ([po_date_var.strftime("%Y%m%d")] if po_date_var else [po_date_var])
    # print(segments)
    ...
    if vn_value_var == "VN00106821":
        segments.append(["N1", "ST", ship_to_name, "91", ghx_ship_to_name_var])
        segments.append(["N1", "RE", company_name_var, "91", "600850213"])
        segments.append(["N3", (address_1_var + " " + address_2_var).rstrip()])
        segments.append(["N4", city_var, state_var, zip_var])
    elif ghx_ship_to_num_var[:4] == "6125" and len(ghx_ship_to_num_var) == 6:
        segments.append(["N1", "BT", bill_to_city_var, "92", ghx_ship_to_num_var])
        ...
        segments.append(["N3", (address_1_var if address_1_var else "").rstrip() + " " + (address_2_var if address_2_var else "").rstrip() + "PO#"])
        segments.append(["N4", ship_to_city_var, ship_to_city_var, ship_to_st_var, ship_to_zip_var])
    else:
        ...
        segments.append(["N2", "Receiving Department"])
        segments.append(["N3", (address_1_var if address_1_var else "" + " " + address_2_var if address_2_var else "").rstrip()])
        segments.append(["N4", city_var, state_var, zip_var])
    segments.append(["N1", "VN", company_name_var, "92", vn_value_var]) if vn_value_var != None and vn_value_var != "71341601" else None
    segments.append(["N1", "SN", "", "92", sn_value_var]) if sn_value_var else None
    if ghx_ship_to_num_var[:4] == "6125" and len(ghx_ship_to_num_var) == 6:
        segments.append(["ITD", "ZZ", "3", "", "", ""]) if (discount_pct_var == '' or discount_pct_var == 0) else segments.append(["", "", "", discount_pct_var, "", discount_days_var])
    else:
        curr_segment = ["ITD"] + (["03"] if "71341601" else ["ZZ"]) + ["3"] + (["", "", ""] if (discount_pct_var == '' or discount_pct_var == 0) else ["", "", "", discount_pct_var, (curr_date + timedelta(days=(int(days_var) if days_var else 0))).strftime("%Y%m%d"), discount_days_var])
        curr_segment += [(curr_date + timedelta(days=(int(days_var) if days_var else 0))).strftime("%Y%m%d"), days_var]
        segments.append(curr_segment + [""]) if (discount_pct_var == '' or discount_pct_var == 0) else [int(discount_pct_var if discount_pct_var else 0 * order_total_var if order_total_var else 0)]

    print("CHECK")
    print((curr_date + timedelta(days=(int(days_var) if days_var else 0))).strftime("%Y%m%d"))
    # print((curr_date + timedelta(days=(int(days_var) if days_var else days=0))).strftime("%Y%m%d"))
    segments.append(["DTM", "011", shipped_date_var.strftime("%Y%m%d") if shipped_date_var else curr_date.strftime("%Y%m%d")])
    ...
    for row in pd_detail.iterrows():
        ...
        if order_details_line_sum_qty_var > 0:
            curr_segment += ["IT1", ghx_line_id_var, order_details_line_sum_qty_var] + ([ghx_uom_var] if ghx_uom_var else [typenex_uom_var if typenex_uom_var else "CA"]) 
            ...
            if ghx_ship_to_num_var[:4] == "6125" and len(ghx_ship_to_num_var) == 6:
                ...
                if buyer_id_var != None:
                    segments.append(curr_segment + [buyer_id_identifier_var if buyer_id_identifier_var else "IN"] + [buyer_id_var, "PL", ghx_line_id_var] if "VN00106821" else [buyer_id_var])
                else:
                    segments.append(curr_segment)
    ...
    if ghx_ship_to_num_var[:4] == "6125" and len(ghx_ship_to_num_var) == 6:
        segments.append(["TDS", int(100*(order_total_var + ship_total_var + (mini_order_surcharge_var if mini_order_surcharge_var else 0)) + saturday_surcharge_var + drop_ship_surcharge_var + (sales_tax_var if sales_tax_var else 0)), ""])
    else:
        segments.append(["TDS", int(100*(order_total_var + ship_total_var + (mini_order_surcharge_var if mini_order_surcharge_var else 0)) + saturday_surcharge_var + drop_ship_surcharge_var + (sales_tax_var if sales_tax_var else 0))] + ([''] if (discount_pct_var == None or discount_pct_var == 0) else ['', '', int(discount_pct_var if discount_pct_var else 0 * order_total_var if order_total_var else 0)]))
    ...
    segments.append(["SE", len(segments)-2, str(group_con_num)[:9] if len(str(group_con_num)) >= 9 else ("").join(['0' for i in range(9-len(str(group_con_num)))])  + str(group_con_num)])
    ...
    print("next")
print("done")
print(["N1", "BT", bill_to_city_var, bill_to_st_var, bill_to_zip_var])